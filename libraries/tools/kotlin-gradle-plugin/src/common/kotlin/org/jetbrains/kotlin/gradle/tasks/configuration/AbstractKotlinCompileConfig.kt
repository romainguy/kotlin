...
internal abstract class AbstractKotlinCompileConfig<TASK : AbstractKotlinCompile<*>>(
    ...
    init {
        ...
        configureTask { task ->
            task.taskBuildCacheableOutputDirectory.value(project.layout.buildDirectory.dir("$KOTLIN_BUILD_DIR_NAME/${task.name}/cacheable"))
                .disallowChanges()
            task.taskBuildLocalStateDirectory.value(project.layout.buildDirectory.dir("$KOTLIN_BUILD_DIR_NAME/${task.name}/local-state"))
                .disallowChanges()
    ...
    constructor(compilation: KotlinCompilationData<*>) : this(
        ...
        configureTask { task ->
            task.friendPaths.from(Callable { compilation.friendPaths })
