...
sealed class MultiFieldValueClassTree<out TI, out TL> {
    ...
    data class InternalNode<out TI, out TL> internal constructor(
        override val type: IrType, val internalNodeValue: TI, val inner: List<TreeField<TI, TL>>
        ...
        constructor(
            ...
            val fieldsByName = replacements.oldFields[primaryConstructor.constructedClass]!!.associateBy { it.name }
        ...
        override fun <RI, RL> map(
            ...
            inner = inner.map { it.map(transformInternalNode, transformLeaf) },
        )
...
class MultiFieldValueClassSpecificDeclarations(
    ...
    val leaves: List<Leaf<Unit>> = ArrayList<Leaf<Unit>>().apply {
        loweringRepresentation.visit(object : MultiFieldValueClassTree.Visitor<Unit, Unit, Unit> {
            ...
            override fun visitInternalNode(treeNode: InternalNode<Unit, Unit>) {
                treeNode.inner.forEach { it.node.visit(this) }
            }
        })
    }.also { if (it.size <= 1) error("${this::class.qualifiedName} for $valueClass must have multiple leaves") }

    private val nodeFullNames: Map<MultiFieldValueClassTree<Unit, Unit>, Name> =
    ...
    private val oldPrimaryConstructor = valueClass.primaryConstructor ?: error("Value classes have primary constructors")
    ...
    val selfImplementationAgnosticDeclarations =
        ImplementationAgnostic(fields.map { VirtualGetter(it.type) { receiver: IrValueParameter -> irGetField(irGet(receiver), it) } })
        ...
        selfImplementationAgnosticDeclarations.nodeToExpressionGetters.filterKeys { it in nodeFullNames }.mapValues { (node, getter) ->
            irFactory.buildProperty {
                ...
                addGetter {
                    ...
                    createDispatchReceiverParameter()
                    body = with(context.createIrBuilder(this.symbol)) {
                        irExprBody(getter.makeGetter(this, dispatchReceiverParameter!!))
                    }
                }
            }
        }

    data class VirtualGetter<T>(val type: IrType, val makeGetter: IrBuilderWithScope.(T) -> IrExpression)
     ...
     */
    inner class ImplementationAgnostic<T>(val virtualFields: List<VirtualGetter<T>>) {
        ...
        val nodeToExpressionGetters: Map<MultiFieldValueClassTree<Unit, Unit>, VirtualGetter<T>> =
            indexesByInternalNode.mapValues { (node, indexes) ->
                VirtualGetter<T>(node.type) { additionalParameter ->
            ...
            } + indexByLeaf.mapValues { (node, index) -> VirtualGetter(node.type) { virtualFields[index].makeGetter(this, it) } }

        // todo type parameters
        // todo create getters
        // todo replace constructor with actual and extract body evals
        // todo recursive
        // todo cheeeck!!!
        // todo equals
        // todo toString
