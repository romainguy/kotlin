...
fun Project.projectTest(
    ...
    maxHeapSizeMb: Long? = null,
    minHeapSizeMb: Long? = null,
    reservedCodeCacheSizeMb: Long? = null,
    body: Test.() -> Unit = {}
): TaskProvider<Test> {
    ...
    return getOrCreateTask<Test>(taskName) {
        ...
        jvmArgs(
            ...
            "-XX:ReservedCodeCacheSize=${reservedCodeCacheSizeMb ?: 256}m",
            "-Djna.nosys=true"
        )

        val memoryPerTestProcessMb: Long = maxHeapSizeMb ?: if (jUnitMode == JUnitMode.JUnit5)
            totalMemoryForTestsMb.coerceAtMost(defaultMaxMemoryPerTestProcessMb * Runtime.getRuntime().availableProcessors())
        else
            defaultMaxMemoryPerTestProcessMb
        ...
        if (parallel && jUnitMode != JUnitMode.JUnit5) {
            val forks = (totalMemoryForTestsMb / memoryPerTestProcessMb).toInt().coerceAtMost(16)
            maxParallelForks =
...
val defaultMaxMemoryPerTestProcessMb: Long = 1600
val reservedMemory = 9000

val totalMemoryForTestsMb: Long
    get() {
        val mxbean = ManagementFactory.getOperatingSystemMXBean() as OperatingSystemMXBean
        return mxbean.totalPhysicalMemorySize / 1048576 - reservedMemory
    }
